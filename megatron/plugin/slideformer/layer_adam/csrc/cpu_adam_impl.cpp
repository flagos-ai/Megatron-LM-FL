...
template <typename ds_params_precision_t, typename ds_grads_precision_t, typename ds_state_precision_t>
void Adam_Optimizer::Step_1(ds_params_precision_t* _params,
                            ds_grads_precision_t* grads,
...
template <class ds_params_precision_t, class ds_grads_precision_t, class ds_state_precision_t>
void create_invoker()
{
    invokers[std::tuple(c10::CppTypeToScalarType<ds_params_precision_t>(),
                        c10::CppTypeToScalarType<ds_grads_precision_t>(),
                        c10::CppTypeToScalarType<ds_state_precision_t>())] =
        step_invoker<ds_params_precision_t, ds_grads_precision_t, ds_state_precision_t>;
}
struct InvokerInitializer {
    InvokerInitializer()
    {
        create_invoker<c10::Half, c10::Half, float>();
        create_invoker<c10::Half, float, float>();
        create_invoker<c10::Half, c10::Half, c10::Half>();
        create_invoker<c10::BFloat16, c10::BFloat16, float>();
        create_invoker<c10::BFloat16, float, float>();
        create_invoker<c10::BFloat16, c10::BFloat16, c10::BFloat16>();
        create_invoker<float, float, float>();
        create_invoker<float, c10::BFloat16, float>();
        create_invoker<float, c10::Half, float>();
    }
            ...
            size_t param_size)
{
    ...
    c10::ScalarType grads_type = at::typeMetaToScalarType(grads.options().dtype());
    c10::ScalarType state_type = at::typeMetaToScalarType(exp_avg.options().dtype());

    auto it = invokers.find(std::tuple(params_type, grads_type, state_type));
    if (it == invokers.end()) {
        throw std::runtime_error("Adam optimizer with param type "s + c10::toString(params_type) +
                                 ", grads type "s + c10::toString(grads_type) +
                 ...
                 torch::Tensor& exp_avg_sq)
{
    ...
    void* exp_avg_sq_ptr = exp_avg_sq.data_ptr();
