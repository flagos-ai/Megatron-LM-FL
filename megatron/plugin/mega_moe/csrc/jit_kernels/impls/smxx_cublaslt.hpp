...
// Batch of GEMMs sharing the same (m, n, k) — creates descriptor/heuristic once,
// then calls cublasLtMatmul repeatedly with different data pointers.
// Each item is (lhs_ptr, rhs_ptr, out_ptr). Layout convention matches cublaslt_gemm.
struct CublasltGemmItem {
    const void* lhs;   // cuBLAS B
    const void* rhs;   // cuBLAS A
    void* out;         // cuBLAS C/D
};

static void cublaslt_gemm_repeated(
        const std::vector<CublasltGemmItem>& items,
        const int& m, const int& n, const int& k,
        const cudaDataType& data_type_a,   // rhs element type
        const cudaDataType& data_type_b,   // lhs element type
        const cudaDataType& data_type_d,   // out element type
        const cute::UMMA::Major& a_major, const cute::UMMA::Major& b_major,
        const int& ld_a, const int& ld_b, const int& ld_d,
        const bool& accumulate) {
    ...
    // Matrix layouts — created once for the shared dimensions
    const auto layout_a = b_major == cute::UMMA::Major::K ? get_cublaslt_layout(data_type_a, k, n, ld_a)
                                                          : get_cublaslt_layout(data_type_a, n, k, ld_a);
    const auto layout_b = a_major == cute::UMMA::Major::K ? get_cublaslt_layout(data_type_b, k, m, ld_b)
                                                          : get_cublaslt_layout(data_type_b, m, k, ld_b);
    const auto layout_d = get_cublaslt_layout(data_type_d, n, m, ld_d);

    // Operation description — created once
    cublasComputeType_t compute_type = CUBLAS_COMPUTE_32F_FAST_TF32;
    cudaDataType_t scale_type = CUDA_R_32F;
    cublasLtMatmulDesc_t desc;
    DG_CUBLASLT_CHECK(cublasLtMatmulDescCreate(&desc, compute_type, scale_type));
    ...
    // Execute all items with the same desc/layout/algorithm
    const float alpha = 1.0, beta = accumulate ? 1.0 : 0.0;
    for (const auto& item : items) {
        DG_CUBLASLT_CHECK(cublasLtMatmul(handle, desc,
                                         &alpha,
                                         item.rhs, layout_a,    // cuBLAS A = rhs
                                         item.lhs, layout_b,    // cuBLAS B = lhs
                                         &beta,
                                         item.out, layout_d,    // C
                                         item.out, layout_d,    // D
                                         &heuristic.algo,
                                         workspace.data_ptr(), workspace_bytes,
                                         stream));
    }
