...
class BridgeCommunicator:
    ...
    def recv_forward(self) -> torch.Tensor:
        ...
        if rank_info.role == CommRole.RECEIVER:
            ...
            shape_tensor = torch.empty((3), device=mg_accelerator.current_device(), dtype=torch.int64)
            dist.broadcast(
        ...
        if rank_info.role == CommRole.SENDER:
            ...
            for dest_rank, grad_shape in zip(rank_info.send_to_ranks, recv_grad_shapes):
                ...
                grad_tensor = torch.empty(
                    grad_shape, device=mg_accelerator.current_device(), dtype=self.comm_dtype
                )
            ...
            shape_tensor = torch.tensor(
                aggregated_gradient.shape, device=mg_accelerator.current_device(), dtype=torch.int64
            )
            ...
            received_gradient = torch.empty(
                received_shape, device=mg_accelerator.current_device(), dtype=self.comm_dtype
            )
        ...
        if rank_info.role == CommRole.SENDER:
            ...
            if num_sends > 0:
                ...
                for i, recv_grad_shape in enumerate(recv_grad_shapes):
                    grad_tensor = torch.empty(
                        recv_grad_shape, device=mg_accelerator.current_device(), dtype=self.comm_dtype
                    )
                    received_gradients_list.append(grad_tensor)
                ...
                shape_tensor = torch.tensor(
                    tensor_shape_to_broadcast, device=mg_accelerator.current_device(), dtype=torch.int64
                )
                dist.broadcast(
    ...
    def send_backward_recv_forward(
        ...
        if rank_info.role == CommRole.RECEIVER:
            ...
            if num_receives > 0:
                ...
                shape_tensor = torch.tensor(
                    tensor_shape_to_scatter, device=mg_accelerator.current_device(), dtype=torch.int64
                )
                dist.broadcast(
        ...
        if rank_info.role == CommRole.SENDER:
            ...
            if tensor_to_send_next is not None:
                ...
                send_shape_tensor = torch.tensor(
                    send_shape, device=mg_accelerator.current_device(), dtype=torch.int64
                )
            ...
            if recv_next:
                for dest_rank in rank_info.send_to_ranks:
                    grad_shape_tensor = torch.empty(
                        (3), device=mg_accelerator.current_device(), dtype=torch.int64
                    )
            ...
            if tensor_to_send_prev is not None:
                ...
                grad_shape_tensor = torch.tensor(
                    grad_shape, device=mg_accelerator.current_device(), dtype=torch.int64
                )
