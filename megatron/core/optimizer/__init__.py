...
def _get_param_groups(
    ...
    # Map (pg_overrides, is_expert_parallel, is_engram_parallel) to params.  # FlagScale Modify
    params_map = {}
    ...
    for model_chunk in model_chunks:
        for name, param in model_chunk.named_parameters():
            ...
            key = (param_override_tuple, is_expert_parallel, is_engram_parallel)  # FlagScale Modify
    ...
    for key in sorted(params_key, key=lambda x: (x[0] is not None, x[0])):
        param_override_tuple, is_expert_parallel, is_engram_parallel = key  # FlagScale Modify
        ...
        param_group = {
            ...
            ######## FlagScale Begin ########
            'is_engram_parallel': is_engram_parallel,
...
def _get_megatron_emerging_optimizer(
    ...
    ######## FlagScale Begin ########
    if any(
        group.get('is_engram_parallel', False)
        for group in all_param_groups
    ):
        raise ValueError(
            "Engram parameters are not supported with emerging optimizers. "
            "Use a standard optimizer such as Adam instead."
        )
    ######## FlagScale End ########
    ...
    if use_separate_distributed_optimizer and any(
        ...
        for model_chunk_idx, model_chunk in enumerate(model_chunks):
            if not hasattr(model_chunk, 'buffers'):
                continue
            # TODO: add engram model chunk to non_layer_wise_buffers.
...
def get_megatron_optimizer(
    ...
    mp_group = [mp_group] if not isinstance(mp_group, list) else mp_group  # FlagScale Modify
    ...
    for dense_model_chunks, overlap_param_gather_with_optimizer_step in zip(
        ...
        param_groups, buffers = _get_param_groups_and_buffers(
            ...
            config=config,
            config_overrides=config_overrides,
            filter_fn=lambda g: not g['is_expert_parallel'] and not g['is_engram_parallel'],  # FlagScale Modify
            buffer_name='buffers',
        )
    ...
    moe_param_groups, moe_buffers = _get_param_groups_and_buffers(
        ...
        config=config,
        config_overrides=config_overrides,
        filter_fn=lambda g: g['is_expert_parallel'] and not g['is_engram_parallel'],  # FlagScale Modify
