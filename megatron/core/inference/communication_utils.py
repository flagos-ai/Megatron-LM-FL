...
def broadcast_from_last_pipeline_stage(
    ...
    if is_last_stage:
        ...
        tensor = torch.empty(size, dtype=dtype, device=mg_accelerator.current_device())
...
def broadcast_list(size, dtype, list_values=None, rank=0, data_parallel=False):
    ...
    if data_parallel:
        if parallel_state.get_model_parallel_src_rank() == torch.distributed.get_rank():
            tensor = torch.tensor(list_values, dtype=dtype, device=mg_accelerator.current_device())
