...
class FullyShardedDataParallel(_BaseDataParallel):
    ...
    def __init__(
        ...
        self.device = device if device else torch.device(cur_platform.current_device_name())  # FlagScale Modify
...
def _load_rng_state_dict(rng_state_dict):
    ...
    cur_platform.set_rng_state(rng_state_dict['cuda_rng_state'])  # FlagScale Modify
