...
class GradReducePipeline:
    ...
    def __init__(
        ...
        rs_stream: Optional[cur_platform.Stream] = None,  # FlagScale Modify
        ...
        if dist_index.use_hybrid_fsdp:
            ...
            self.outer_fsdp_group_grad_reduce_stream = cur_platform.Stream()  # FlagScale Modify
        else:
    ...
    def _bucket_group_gradient_reduce(
        ...
        with cur_platform.stream(reduce_scatter_stream):  # FlagScale Modify
...
class AllGatherPipeline:
    ...
    def __init__(
        self,
        param_and_grad_buffer: ParamAndGradBuffer,
        ag_stream: Optional[cur_platform.Stream] = None,  # FlagScale Modify
    ) -> None:
        self.buffer = param_and_grad_buffer
        ...
        if (
            ...
            self.outer_fsdp_group_param_gather_stream = cur_platform.Stream()  # FlagScale Modify
    ...
    def all_gather_params(
        ...
        for _, buckets in bucket_group_to_buckets.items():
            ...
            with cur_platform.stream(all_gather_stream):  # FlagScale Modify
...
def check_gpu_memory(threshold=0.9):
    ...
    """
    if not cur_platform.is_available():  # FlagScale Modify
        return False
    ######## FlagScale Begin ########
