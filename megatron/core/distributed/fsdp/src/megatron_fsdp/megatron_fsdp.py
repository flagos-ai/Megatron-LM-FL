...
class MegatronFSDP(torch.nn.Module):
    ...
    def __init__(
        ...
        self.device = (
            device if device is not None else torch.device(mg_accelerator.current_device_name())
        )
        if self.device != torch.device(mg_accelerator.current_device_name()):
            logger.warning(
                f"[Rank {torch.distributed.get_rank()}] Megatron-FSDP is "
                f"using device {self.device} instead of the current device "
                f"{torch.device(mg_accelerator.current_device_name())}, "
    ...
    def _init_fsdp_param_and_grad_buffer(self):
        ...
        self.side_stream_for_buffer_copy_and_grad_accum = mg_accelerator.Stream()
        self.side_stream_for_param_gather = mg_accelerator.Stream()
