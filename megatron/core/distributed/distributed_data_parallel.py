...
class DistributedDataParallel(_BaseDataParallel):
    ...
    def __init__(
        ...
        all_params = []
        ########## FlagScale Begin ##########
        # Engram embedding params are reduced over a dedicated engram data-parallel group,
        # so they are collected separately and never mixed into the standard/expert buffers.
        ...
        for name, param in self.module.named_parameters():
            ...
            ########## FlagScale Begin ##########
            if self.engram_dp_group is not None and getattr(
        ...
        ########## FlagScale Begin ##########
        engram_buffer_groups = (
            group_params_for_buffers(engram_embedding_params, self.ddp_config.grad_reduce_in_fp32)
        ...
        ########## FlagScale Begin ##########
        # Allocate separate param+grad buffers for engram embedding parallel params, reduced
        # over the dedicated engram data-parallel group instead of the standard/expert groups.
        self.engram_embedding_buffers = []
        for buffer_key, (params, param_indices) in engram_buffer_groups.items():
            params_with_names = [(p, param_to_name[p]) for p in params]
            engram_buffer = _ParamAndGradBuffer(
                self.ddp_config,
                buffer_key.param_dtype,
                buffer_key.grad_dtype,
                params_with_names,
            ...
            self.engram_embedding_buffers.append(engram_buffer)
        ########## FlagScale End ##########
        ...
        if self.ddp_config.num_distributed_optimizer_instances > 1:
            ...
            for bucket_groups in [
                ...
                self.engram_embedding_bucket_groups,  # FlagScale Add
            ]:
                communication_stream = cur_platform.Stream(
