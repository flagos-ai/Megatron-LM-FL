...
def get_gpt_layer_with_transformer_engine_submodules(
    ...
    enable_hyper_connection: bool = False,  # FlagScale Modify
) -> TransformerLayerSubmodules:
...
def get_gpt_decoder_layer_specs(
    ...
    dualpipev_stage: Optional[bool] = None,  # FlagScale Modify
) -> TransformerBlockSubmodules:
    """GPT block spec."""
...
def get_gpt_mtp_block_spec_for_backend(
    ...
    if not config.mtp_use_repeated_layer:
        offset = get_mtp_layer_offset(config, vp_stage=vp_stage, dualpipev_stage=dualpipev_stage)  # FlagScale Modify
