...
class YarnRotaryEmbedding(RotaryEmbedding):
    ...
    def __init__(
        ...
        device = 'cpu' if use_cpu_initialization else mg_accelerator.current_device()
    ...
    def forward(self, max_seq_len: int, offset: int = 0, packed_seq: bool = False) -> Tensor:
        ...
        if self.inv_freq_extra.device.type == 'cpu':
            ...
            self.inv_freq_extra = self.inv_freq_extra.to(device=mg_accelerator.current_device())

        if self.inv_freq_inter.device.type == 'cpu':
            # move `inv_freq_inter` to GPU once at the first micro-batch forward pass
            self.inv_freq_inter = self.inv_freq_inter.to(device=mg_accelerator.current_device())
