...
_BACKEND_MODULE_NAME_BY_BACKEND = {
    ...
    "triton": "megatron.plugin.dsa_kernel.triton_dsa_kernels",
}
...
def _get_dsa_kernel_backend(config: TransformerConfig) -> str:
    ...
    backend = config.dsa_kernel_backend or "none"
    if backend != "none" and backend not in _BACKEND_MODULE_NAME_BY_BACKEND:
        raise ValueError("dsa_kernel_backend must be one of: none, cudnn, triton")
    return backend
...
def _load_backend(config: TransformerConfig) -> Optional[ModuleType]:
    ...
    try:
        ...
        raise RuntimeError(
            f"Failed to import DSA kernel backend '{backend}' ({module_name})."
        ) from exc
    _BACKEND_SELECTION = module_name
    _LOGGER.debug("DSA backend resolved: requested=%s provider=%s", backend, module_name)
    return _BACKEND


def _log_declined_hook(config: TransformerConfig, hook_name: str, reason: str) -> None:
    """Log when the selected provider delegates a hook to its native continuation."""
    if not _LOGGER.isEnabledFor(logging.DEBUG):
        return
    _LOGGER.debug(
        "DSA backend %s delegated %s to its native continuation (%s).",
...
def _resolve_fused_hook(config: TransformerConfig, hook_name: str):
    ...
    ``None`` means either that the explicit ``none`` implementation owns the
    whole path, or that the selected provider intentionally delegates this
    operation to the caller's PyTorch-native continuation. That continuation is
    part of the provider contract; it never triggers selection of another backend.
    """
