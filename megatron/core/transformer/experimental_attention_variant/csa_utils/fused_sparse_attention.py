...
def _compute_full_csa_teacher_lse(
    ...
    *,
    ...
    """Return the full CSA teacher LSE for the selected backend.

    The Triton backend uses its streaming kernel when supported. Other backends,
    and unsupported Triton layouts, use the native PyTorch implementation that
    is part of their declared implementation chain; a cuDNN selection never
    probes or launches the Triton teacher-LSE kernel.
    """
    if _DSA_FORCE_TRITON and can_use_fused_csa_teacher_lse(
...
def compute_sparse_attn_target_head_sum(
    q_attn: Tensor, k_attn: Tensor, lse: Tensor, topk_indices: Tensor, softmax_scale: float
) -> Tensor:
    """Sum the UNNORMALISED sparse-teacher probability mass over local heads.

    The returned tensor must be summed across tensor-parallel ranks before it
    is L1-normalised. Shapes follow the ``det`` layout convention used by
    :func:`_compute_attn_target`:

    * SBHD: ``q_attn (b, sq, np, d)`` BSHD with ``k_attn (b, sk, d)`` and
      ``lse (b, sq, np)`` (the full sparse denominator, window+sink+topk);
      ``topk_indices (b, sq, topk)``. Returns ``(b, sq, topk)``, the layout
      expected by :func:`_kl_loss_from_target_predict` (matches the real
      ``sparse_attn_score_recompute_wrapper.target``).
    * THD: ``q_attn (total_q, np, d)`` with flat-global ``k_attn (total_k, d)``,
      ``lse (total_q, np)`` and ``topk_indices (total_q, topk)``.
      Returns ``(total_q, topk)``.
    """
    is_thd = q_attn.ndim == 3
    if is_thd:
        q_bshd = q_attn.unsqueeze(0)
        k_bshd = k_attn.unsqueeze(0)
        lse_bsh = lse.unsqueeze(0)
        topk_bst = topk_indices.unsqueeze(0)
    else:
        q_bshd, k_bshd, lse_bsh, topk_bst = q_attn, k_attn, lse, topk_indices
        if topk_bst.ndim == 3 and topk_bst.shape[:2] != q_bshd.shape[:2]:
            # The SBHD call site passes (sq, b, topk); normalize to (b, sq, topk).
            topk_bst = topk_bst.permute(1, 0, 2)

    b, sq, np_, d = q_bshd.shape
    safe = topk_bst.long().clamp_min(0)
    batch_ids = torch.arange(b, device=q_bshd.device)[:, None, None]
    selected_k = k_bshd[batch_ids, safe]  # (b, sq, topk, d)
    scores = torch.einsum("bshd,bstd->bsht", q_bshd.float(), selected_k.float())
    probs = torch.exp(scores * softmax_scale - lse_bsh.float().unsqueeze(-1))
    head_sum = probs.sum(dim=2).masked_fill(topk_bst < 0, 0.0)  # (b, sq, topk)
    if is_thd:
        return head_sum.squeeze(0)
    return head_sum


def all_reduce_tp_target_mass(target_mass: Tensor, tp_group) -> Tensor:
    """Sum the unnormalised target mass over TP ranks in place."""
...
class FusedCSAIndexerSparseAttnFunc(torch.autograd.Function):
    ...
    * **Dense loss** (``sparse_loss=False``, the default) — KL is
    ...
    def forward(
        ...
        tp_group=None,  # tensor-parallel group; sums teacher mass across local-head shards
    ) -> Tuple[Tensor, Tensor]:
        ...
        if sparse_loss:
            ...
            if tp_group is not None and tp_group.size() > 1:
                ...
                all_reduce_tp_target_mass(target_mass, tp_group)
                target = target_mass / target_mass.sum(dim=-1, keepdim=True).clamp(
                    min=_CLIP_PROB_MIN
                )
            else:
            ...
            if tp_group is not None and tp_group.size() > 1:
                # Sum the unnormalised per-row attention mass across the
                # local-head shards, then recompute the L1 norm once.
                all_reduce_tp_target_mass(attn_score, tp_group)
    ...
    def backward(ctx, grad_output, grad_loss):
        ...
        #   compressed_kv, cu_seqlens_q_unpadded, tp_group
        return (
            grad_query,
            grad_kv_full,
            d_sink,
