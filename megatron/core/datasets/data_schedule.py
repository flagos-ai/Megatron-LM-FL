    ...
    def reroute_samples_to_hdp_ranks(
        ...
        def _pack_sample_by_key(key: str) -> torch.Tensor:
            ...
            return (
                ...
                else torch.empty(0, device=cur_platform.current_device(), dtype=batch[0][key].dtype)  # FlagScale Modify
            )
        ...
        for key in data_keys:
            ...
            recv_tensor = torch.empty(
                sum(recv_lens_split), device=cur_platform.current_device(), dtype=send_tensor.dtype  # FlagScale Modify
            )
