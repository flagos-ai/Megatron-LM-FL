...
_D1_ETP2 = (
    _FIXTURES / "flagscale_dual_node_deepseek_d1_tp2_sp_etp2_mock.yaml"
)
...
def test_deepseek_d1_etp2_fixture_only_applies_the_reviewed_etp2_derivation() -> None:
    expected = deepcopy(_load(_D1_ETP1))
    derived = _load(_D1_ETP2)
    expected["experiment"]["exp_name"] = (
        "megalens-g7-9-deepseek-d1-tp2-sp-etp2-mock"
    )
    expected["train"]["system"]["expert_tensor_parallel_size"] = 2

    assert derived == expected


def test_deepseek_d1_etp2_fixture_derives_expert_dp1() -> None:
    config = _load(_D1_ETP2)
    runner = config["experiment"]["runner"]
    system = config["train"]["system"]
    model = config["train"]["model"]
    ...
    assert (world_size, model_dp, expert_dp, microbatches) == (16, 4, 1, 1)
    assert (
        system["tensor_model_parallel_size"],
        system["pipeline_model_parallel_size"],
        system["context_parallel_size"],
    ...
    assert model["moe_token_dispatcher_type"] == "alltoall"
    assert model["disable_bias_linear"] is True
    assert config["experiment"]["envs"]["NCCL_NVLS_ENABLE"] == 0


def test_deepseek_d1_etp2_fixture_stays_out_of_the_single_node_runner() -> None:
    assert _D1_ETP2.stem not in single_node_gate._CONFIG_PROFILES
