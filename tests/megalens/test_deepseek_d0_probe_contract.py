...
def _write_d0_dp4_ep2_profile(trace_root: Path) -> None:
    _write_profile(
        trace_root,
        data_parallel_size=contract.D0_REDUCED_DATA_PARALLEL_SIZE,
        expert_model_parallel_size=(
            contract.D0_REDUCED_EXPERT_MODEL_PARALLEL_SIZE
        ),
        include_dp_groups=True,
        include_etp1_metadata=True,
    )
...
def test_deepseek_d0_dp4_ep2_contract_accepts_the_reduced_runtime_trace(
    tmp_path: Path,
) -> None:
    trace_root = tmp_path / "traces"
    _write_d0_dp4_ep2_profile(trace_root)

    assert (
        contract.validate_deepseek_d0_dp4_ep2_trace(
            trace_root,
            microbatches_per_iteration=_TEST_MICROBATCHES,
        )
        == ()
...
def test_deepseek_d0_dp4_ep2_contract_requires_the_expert_dp2_group(
    tmp_path: Path,
) -> None:
    trace_root = tmp_path / "traces"
    _write_d0_dp4_ep2_profile(trace_root)
    ...
    failures = contract.validate_deepseek_d0_dp4_ep2_trace(
        trace_root,
        microbatches_per_iteration=_TEST_MICROBATCHES,
    )

    assert "trace.deepseek_d0_dp4_ep2.dp_group" in {
        failure.code for failure in failures
    }


def test_deepseek_d0_dp4_ep2_contract_requires_the_ep2_metadata_group(
    tmp_path: Path,
) -> None:
    trace_root = tmp_path / "traces"
    _write_d0_dp4_ep2_profile(trace_root)
    ...
    assert "trace.deepseek_d0_dp4_ep2.metadata_collective" in {
        failure.code for failure in failures
    }
