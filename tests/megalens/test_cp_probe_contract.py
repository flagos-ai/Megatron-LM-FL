...
def _write_cp_trace(
    ...
    *,
    ...
    for iteration in (1, 2):
        ...
        event("grad-sync", "B", schedule="no-pipelining", timing_phase="framework_phase")
    ...
    path = trace_root / f"benchmark-global-{rank}-data-{data_rank}-pipeline-0-tensor-0.json"
...
def _write_terminal_checkpoint(run_root: Path) -> None:
    ...
    (checkpoint_root / "latest_checkpointed_iteration.txt").write_text("2", encoding="utf-8")
    (iteration_root / "common.pt").write_bytes(b"checkpoint")


def test_cp2_fixture_selects_the_supported_te_topology() -> None:
...
def test_cp2_contract_requires_exactly_one_nested_dp_allreduce(tmp_path: Path) -> None:
    ...
    begin = next(row for row in rows if row.get("name") == "dp-allreduce" and row.get("ph") == "B")
    end = next(row for row in rows if row.get("name") == "dp-allreduce" and row.get("ph") == "E")
...
def test_cp_contract_rejects_speculative_cp_events_and_fields(tmp_path: Path) -> None:
    ...
    rows = json.loads(first_path.read_text(encoding="utf-8"))
    forward_begin = next(
        row for row in rows if row.get("name") == "forward-step" and row.get("ph") == "B"
    )
...
def test_cp2_contract_requires_equal_dp_payload_sizes(tmp_path: Path) -> None:
    ...
    assert "trace.cp.dp_payload_consistency" in {failure.code for failure in failures}
...
def test_cp2_profile_requires_rank_fields_on_existing_events(tmp_path: Path) -> None:
    ...
    report = manifest.validate_trace(tmp_path, gate.PROFILES["cp2-te"], trace_enabled=True)
...
def test_cp2_training_contract_requires_the_launcher_cp_size(tmp_path: Path) -> None:
    ...
    failures = training_run_contract.validate_two_iteration_transformer_engine_cp2_checkpoint(
        tmp_path, False
    )
    assert {failure.code for failure in failures} == {"run.training.context_parallel_size"}
