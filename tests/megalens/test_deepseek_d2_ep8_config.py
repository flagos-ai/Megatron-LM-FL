...
_D2_EP8 = _FIXTURES / "flagscale_dual_node_deepseek_d2_ep8_mock.yaml"
...
def test_deepseek_d2_ep8_fixture_only_applies_the_reviewed_derivation() -> None:
    expected = deepcopy(_load(_BASELINE))
    derived = _load(_D2_EP8)
    expected["experiment"]["exp_name"] = "megalens-g7-9-deepseek-d2-ep8-mock"
    expected["experiment"]["runner"]["nproc_per_node"] = 8
    expected["experiment"]["envs"]["CUDA_VISIBLE_DEVICES"] = (
        "0,1,2,3,4,5,6,7"
    )
    expected["train"]["system"]["expert_model_parallel_size"] = 8
    expected["train"]["model"]["global_batch_size"] = 8

    assert derived == expected


def test_deepseek_d2_ep8_fixture_derives_the_reviewed_parallel_topology() -> None:
    config = _load(_D2_EP8)
    ...
    assert (world_size, model_dp, expert_dp, microbatches) == (16, 8, 1, 1)
    assert (
        system["tensor_model_parallel_size"],
        system["pipeline_model_parallel_size"],
        system["context_parallel_size"],
...
def test_deepseek_d2_ep8_fixture_stays_out_of_the_single_node_runner() -> None:
    assert _D2_EP8.stem not in single_node_gate._CONFIG_PROFILES
