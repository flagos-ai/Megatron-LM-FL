...
"""Trace contract for the controlled TP2/EP4 Flex+HybridEP profile."""
...
_TOPOLOGY = {
    ...
    # while model TP is 2, so the HybridEP group contains the four EP ranks.
    "group_size": 4,
    "ep_size": 4,
    "tp_size": 1,
}
...
def _failure(code: str, message: str, rank: int, iteration: int | None) -> Failure:
    ...
    return Failure(f"trace.moe_flex_hybridep.{code}", message, evidence)
...
def _load_iterations(trace_root: Path) -> Mapping[int, Sequence[Iteration]]:
    ...
    for rank, content in collect_benchmark_files(trace_root):
        if rank.global_rank is None or rank.global_rank in result:
            raise ValueError("Flex+HybridEP contract requires one shard per global rank")
...
def validate_tp2_ep4_flex_hybridep(trace_root: Path) -> tuple[Failure, ...]:
    """Validate two fused HybridEP dispatch/combine calls per iteration and rank."""
...
__all__ = ["validate_tp2_ep4_flex_hybridep"]
