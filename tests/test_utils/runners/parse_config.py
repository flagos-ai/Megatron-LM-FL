...
def get_platform_config(platform, device=None):
    ...
    if not platform:
        raise ValueError(
            "Platform must be specified. Available platforms: cuda, metax. See template.yaml for creating new platforms."
        )
    ...
    # If platform is a metax device type (c500) and no device specified
    if platform in ["c500"] and device is None:
        device = platform
        platform = "metax"
...
def get_device_types(platform):
    ...
    """
    if not platform:
        raise ValueError("Platform must be specified. Available platforms: cuda, metax")

    script_dir = os.path.dirname(os.path.abspath(__file__))
    platform_file_map = {"cuda": "cuda.yaml", "metax": "metax.yaml"}
