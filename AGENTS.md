...
- `G*` 用于训练路径 Probe，以及 Roadmap 明确定义的训练路径共存或兼容性验收；`A*` 用于
  Trace 聚合与并行 Analysis，`V*` 用于跨功能复用的扫描、门禁、runner、训练配置和证据清单，
  `E*` 用于打包、FlagScale 覆盖层和工作环境。
- 若 `G*` 或 `A*` 独立验收项明确以共存或兼容性验证为交付内容，该项专用的 fixture、精确契约
  和运行证据可以复用对应编号；状态只按原定验收范围更新，不计为新增 Probe、事件字段或
  Analysis。
